...
commonIndex(const ITensorT<IndexT>& A,
    ...
    {
    for(auto& I : A.inds())
        if( hasindex(B.inds(),I) )
            {
            return I;
            }
    return IndexT{};
    }
template Index commonIndex(const ITensorT<Index>& A, const ITensorT<Index>& B);
template IQIndex commonIndex(const ITensorT<IQIndex>& A, const ITensorT<IQIndex>& B);
...
template Index uniqueIndex(const ITensorT<Index>& A, const ITensorT<Index>& B);
template IQIndex uniqueIndex(const ITensorT<IQIndex>& A, const ITensorT<IQIndex>& B);
...
template ITensorT<Index> swapPrime(ITensorT<Index>, int, int);
template ITensorT<IQIndex> swapPrime(ITensorT<IQIndex>, int, int);
...
multSiteOps(ITensorT<I> A, ITensorT<I> const& B) 
    ...
    A.prime();
    A *= B;
    // TODO: make sure only Site indexes
    // are mapprimed
    A.mapprime(2,1);
    return A;
    }
